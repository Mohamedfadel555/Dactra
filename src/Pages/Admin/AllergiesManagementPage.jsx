...
export default function AllergiesManagementPage() {
  ...
  const addMutation = useMutation({
    mutationFn: (payload) => adminAPI.addAllergy(payload),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, payload }) => adminAPI.updateAllergy(id, payload),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => adminAPI.deleteAllergy(id),
    onSuccess: () => {
    ...
    onError: (error) => {
      ...
      console.error("Delete allergy error:", error?.response?.data || error);
  ...
  const handleDelete = (item) => {
    if (!window.confirm(`Delete allergy "${item.name}"?`)) return;
    if (!item?.id) {
      toast.error("Allergy ID not found");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1">
            Add, edit, or remove available allergies shown in signup and profile.
          </p>
        </div>
      </div>
      ...
      >
        <h2 className="text-sm sm:text-base font-semibold text-gray-800">
          {editing ? "Edit Allergy" : "Add New Allergy"}
        </h2>
